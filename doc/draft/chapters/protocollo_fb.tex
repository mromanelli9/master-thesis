...
\begin{algorithmic}[1]
	\ForEach{turn}
		...
		\If{$\neg$ (heardHelloMsg() $\vee$ heardCollision())}
...
\begin{algorithmic}[1]
	\State{broadcastMsg.declaredMaxRange $\gets$ max(lmr, cmr)}
	\State{broadcastMsg.position $\gets$ getPosition()}
	\State{broadcast(broadcastMsg)}
\end{algorithmic}
\end{algorithm}
%
\begin{algorithm}[!h]
...
\begin{algorithmic}[1]
	...
	\State{rcw $\gets$ random(cwnd)}
	\State{wait(rcw)}
	\If{sameMessageHeardFromBack()}
		\State{exit()}
	\ElsIf{sameMessageHeardFromFront()}
		\State{restartBroadcastProcedure()}
	\Else{}
		\State{broadcastMsg.maxRange $\gets$ max(LMR, CMR)}
		\State{broadcastMsg.position $\gets$ getPosition()}
		\State{broadcast(broadcastMsg)}
	\EndIf{}
\end{algorithmic}
