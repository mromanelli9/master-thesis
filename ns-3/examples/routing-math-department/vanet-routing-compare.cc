...
VanetRoutingExperiment::Run ()
{
  ...
  CheckThroughput ();
	...
	std::string animFile = "outputs/myvanet-animation.xml";
	AnimationInterface anim (animFile);
	anim.SetMobilityPollInterval (Seconds (0.250));
	anim.EnablePacketMetadata (true);
	anim.EnableIpv4L3ProtocolCounters (Seconds (2), Seconds (m_TotalSimTime));
	anim.EnableWifiMacCounters (Seconds (2), Seconds (m_TotalSimTime));
	anim.EnableWifiPhyCounters (Seconds (2), Seconds (m_TotalSimTime));
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 3)
    {
      ...
      m_logFile = "outputs/myvanet.log";
      m_mobility = 1;
      m_nNodes = 22;
      m_TotalSimTime = 50;
      m_nodeSpeed = 0;
      m_nodePause = 0;
      m_CSVfileName = "outputs/myvanet.output.csv";
      m_CSVfileName = "outputs/myvanet.output2.csv";
      ...
      if (m_loadBuildings != 0)
        {
          std::string bldgFile = "inputs/Blocco-IME.poly.xml";
