...
ObstacleShadowingPropagationLossModel::ObstacleShadowingPropagationLossModel (Topology *topo)
  : PropagationLossModel ()
{
	this->topology = topo;
}
...
ObstacleShadowingPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  NS_ASSERT(this->topology != 0);

  if (this->topology->HasObstacles() == true)
    {
      ...
      L_obs = this->topology->GetObstructedLossBetween(p1, p2, r);
    }
